...
See `skills/README.md` for the complete guide, including SKILL.md format and the `bin/` directory pattern for shell scripts.
...
- **`bin/` directory for shell scripts** — ZeroClaw's security policy rejects `.sh` files inside skill packages during audit. Scripts go in `bin/` at the repo root.
